...
exports.run = async (client, message, args) => {
  ...
  if(!args[0]) {
    message.channel.send('**:x: | Ayarlamak istediğin sayı girmelisin. a!sayaç 100 #sayaç**')
    return
  }
  
  if(!sayackanal) {
    message.channel.send('**:x: | Ayarlamak istediğin kanalı girmelisin. ``a!sayaç 100 #sayaç``**')
  }
  ...
  if(args[0] === "kapat") {
    ...
    message.channel.send(`**:white_check_mark: | Sayaç başarıyla sıfırlandı.| Tekrar açmak için: \`a!sayaç <HedefSayı> <#YazıKanalı>**`)
    return
  }
  
  if(isNaN(args[0])) {
    message.channel.send(` Örnek kullanım: a!sayaç <HedefSayı> <#YazıKanalı>`)
    return
  }
  ...
  message.channel.send(`**:white_check_mark: | Sayaç \`${args[0]}\` olarak ayarlandı! | Sayaç kanalı ${sayackanal} olarak ayarlandı.| Sayaç komudunu kapatmak için \a!sayaç kapat yazınız!**`)
}
