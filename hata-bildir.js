...
let mesaj = args.slice(0).join(' ');
  if (mesaj.length < 1) return message.channel.send(new Discord.RichEmbed().setTitle(`AloneBot - Hata Bildirimi`).setColor("RANDOM").setDescription('\`\`\`Doğru Kullanım :: a!hatabildir hata Gibi!\`\`\` '));
...
const tavsiye = new Discord.RichEmbed()
  .setTitle(`AloneBot - Hata Bildirimi`)
  .setColor("RANDOM")
...
return client.channels.get("667346023630503946").send(tavsiye);
};
